...
from that template proved it.** It patched a script vendored from here, wrote
...
An adopter of that template hit exactly that and answered it with a shape worth
taking:
...
own. Re-read the upstream at its new commit, diff it against the pin, and take
the change as its own entry with its own acceptance.
