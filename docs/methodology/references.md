...
```bash
sh `cargo xtask mine` OWNER/REPO --out references
```

```bash
pwsh -NoProfile -File `cargo xtask mine` OWNER/REPO -Out references
```
...
bot, ⛔ **and not the operator.**
[`../security/remote-ops.md`](../security/remote-ops.md) carries the rule and
what it cost: two findings behind it were correct in substance and stale in
detail, because a claim describes the tree it was written against.
