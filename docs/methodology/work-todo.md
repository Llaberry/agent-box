...
⭐ **This project uses this model and no other.** The alternative, a numbered
sequence of units each with its own plan and handoff, was not taken: the work
here is a set of independent items that can be done in several orders, and the
valuable question is what matters most rather than what comes next in a
sequence. [`authoring.md`](authoring.md) says how an entry is written.
...
- **Source.** Where the idea came from. ⚠ Provenance, not a path a reader must
  be able to open. "The operator", "found while measuring another entry", or a
  citation.
