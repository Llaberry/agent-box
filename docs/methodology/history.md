...
⛔ **An agent working from the template this repository started from wrote the project's history into every
...
⭐ **Two independent projects built from the template this repository started from invented the same
...
the evidence for keeping it here: it is not a preference, it is the
...
everywhere. `cargo xtask check markers` reads this directory like any
other.
