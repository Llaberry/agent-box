...
⭐ **A procedure, not a dependency.** No gate needs a container. A check that
can use one **exits 2** when there is none, because "could not run" and
"failed" are different facts. A host with no container engine is not a failing
build.

⚠ **This project is different from most in one way, and it matters here.** A
container is not only a place to measure in: it is one of the backends this
project drives. Do not let the two uses blur. This page is about throwing a
machine away after asking it a question. What the product does with podman is
