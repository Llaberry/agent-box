...
| the work entries | one per item, in the record directory, each closing in place with its own acceptance command |
...
narrative.** An agent working from the template this repository started from wrote its project's history
...
Four rules, and `cargo xtask check changelog`
