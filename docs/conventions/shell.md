...
```bash
node `cargo xtask gate`.mjs
```

```bash
pwsh -NoProfile -File `cargo xtask gate`
```
...
- a `.gitattributes` that states the rule per type, which
  [`../../.gitattributes`](../../.gitattributes) is;
...
defence, and the one used here, is to write no here-strings in a
`.ps1` at all.
...
- ⛔ **A byte class is not a character class, and the wrong one is silently
  ...
  `cargo xtask check control-bytes`,
...
- ⛔ **Every invocation carries `-NoProfile`, and this repository never said
  ...
  ```bash
  pwsh -NoProfile -File `cargo xtask check gate`
  ```
...
- ⛔ **`[int]` on a double rounds.** `[int]` applied to `2.65` gives 3, so a 2h39m session
