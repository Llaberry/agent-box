...
The template carries the comparison of the tools that left.
...
## Shell checks run on any host with a POSIX shell
...
⭐ **This project does not carry PowerShell twins.** The Windows daemon runs inside WSL2, where a POSIX shell is present, so one implementation is enough.
...
The twin runner is not carried here: without PowerShell twins there is nothing to compare.
...
⛔ **This project does not take that route yet, and the reason is the one in the
...
runner invokes `check-twins`, so comparing the two runners would recurse.
...
Not carried here. The Windows daemon runs inside WSL2, where a POSIX shell is present, so one implementation is enough.
...
[`.githooks/commit-msg`](../.githooks/commit-msg) is what
...
Does `CHANGELOG.md` still obey the four machine-held rules: order, dates, record pointers, deploy lines.
...
⭐ **`--selftest` proves the joiner against a built-in fixture.**
